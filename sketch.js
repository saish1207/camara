  ...
  invisibleGround.x = camera.position.x
   ...
   // ground.velocityX = -(4 + 3* score/100)
    ...
    if (camera.position.x < 0){
      camera.position.x  = camera.position.x.width/2;
    }
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   ...
   obstacle.x =  camera.position.x -(4 + 3* score/100) + width;
   //obstacle.y = ground.y
...
function spawnClouds() {
 ...
 if (frameCount % 200 === 0) {
    ...
    cloud.scale = 0.5;
    cloud.x =  camera.position.x + width;
